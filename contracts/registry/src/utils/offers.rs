...
pub fn set_sale_offer(e: &Env, domain: &Domain, amount: &u128) {
    ...
    e._offers().bump(&OffersDataKeys::SaleOffer(domain.node.clone()));
}
...
pub fn set_new_buy_offer(e: &Env, core_data: &CoreData, caller: &Address, domain: &Domain, amount: &u128) {
    ...
    token::Client::new(&e, &core_data.col_asset).transfer(&caller, &e.current_contract_address(), &(amount.clone() as i128));

    e._offers().set(&Offer::BuyOffer(new_offer));
    e._offers().bump(&OffersDataKeys::BuyOffer(domain.node.clone()));
}
...
pub fn update_buy_offer(e: &Env, core_data: &CoreData, caller: &Address, old_buy_offer: &BuyOffer, domain: &Domain, amount: &u128) {
    ...
    if caller == &old_buy_offer.buyer {
        ...
        token::Client::new(&e, &core_data.col_asset).transfer(&caller, &e.current_contract_address(), &(extra_amt as i128));
    } else {
